...
# Varijanta: GF (Graph Factorization) – faktorizacija matrice susedstva

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data. 
GF = low-rank faktorizacija A ≈ Y Y^T; ovde SVD(A), Y = U sqrt(S)
"""
...
def compute_gf_embeddings(M, k=EMBED_DIM):
...
def main():
    ...
    emb = compute_gf_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GF + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (GF + kvantna regresija, deterministički, strukturno):
    (8, 11, 23, 26, 33, 34, 35)
    """
...
A = matrica susedstva M[1:40, 1:40] (bez self-loop-ova).

SVD(A) → embedding = U[:, :k] * sqrt(S[:k]) 
(niska rang faktorizacija A ≈ Y Y^T, Y = U sqrt(S)).
